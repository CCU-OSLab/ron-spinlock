...
static __thread int idxary[NUM_CORES];
...
ron_mutex_t *ron_mutex_create(const pthread_mutexattr_t *attr) {
    ...
    memset((void *)impl->wait_array, 0, NUM_CORES * sizeof(union extended_atomic));
...
void ron_thread_start(void) {
	int count = atomic_fetch_add_explicit(&thread_count, 1, memory_order_release)%NUM_CORES;
    cpu_set_t set;
    CPU_ZERO(&set);
    CPU_SET(count, &set);
